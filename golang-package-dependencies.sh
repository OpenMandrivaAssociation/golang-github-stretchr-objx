...
cd "`dirname $0`"
NAME="`grep ^Name golang-github-stretchr-objx.spec |cut -d: -f2- |xargs echo`"
VERSION="`grep ^Version golang-github-stretchr-objx.spec |cut -d: -f2- |xargs echo`"
URL="`grep ^URL golang-github-stretchr-objx.spec |cut -d: -f2- |xargs echo`"
SOURCE="`grep ^Source0 golang-github-stretchr-objx.spec |cut -d: -f2- |sed -e "s,%{name},$NAME,g; s,%{version},$VERSION,g;" |xargs echo`"
